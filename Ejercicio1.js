...
/* La funcion busca en el directorio las empresas que coincidan con la descripción del servicio 
   y el nombre de la empresa*/

function busqueda(descServicio, emp = "AEF"){
    // Devuelve un array con las empresas que cumplen el criterio antes mencionado
    const corrida = corridasWeb.filter(c => c.empresa === emp && c.descripcionServicio === descServicio);    
    // Se evalua que exitan empresas en el array del filtro para mostrar los valores solicitados

    // De lo contrario devulve un mensaje de que no hay corridas que satisfagan los criterios
    if (corrida.length === 0){
        return console.log("Error, criterios no cumplidos");
    }else{
        
        for (let i of corrida){
            
            // Quería probar si esto funcionaba pero quizá mi idea estaba mal y solo lo compliqué jaja xD 
            // no le entendí a lo mejor 

            final = { "a": i.numeroCorrida, 
                      "b": i.descripcionServicio,
                      "c": i.empresa, 
                      "d": i.descripcionEmpresa, 
                       e: i.listaDescuentos === undefined ? false:true
                    };
            
            arrayfinal.push(final);
            
            //final.push([i.numeroCorrida, i.descripcionServicio, i.empresa, i.descripcionEmpresa, i.listaDescuentos === undefined ? false:true]);
        }
         return arrayfinal;
        //return final;        
    }
...
const l = busqueda("ECONOMICO");

/* Cuando busco con valores que no cumplen el criterio me muestra un error de que trato de iterar un undefined
   creo que es parte igual de por como lo resolví jaja me gustaría retroalimentación sobre eso
*/
l.forEach(v => console.log("Numero de corrida: " + v.a + "\n" + "Servicio: " + v.b + "\n" + "Empresa: " + v.c + "\n" + "Descripción: " + v.d + "\n" + "Promoción: " + v.e));